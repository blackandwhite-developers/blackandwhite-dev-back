...
const getReservationDetailPathValidator = yup.object({
  id: yup.string().matches(REGEX.EMPTY_VARIABLE_PATH, 'reservationId는 필수 입력값입니다.').required(),
});
...
const createReservationBodyValidator = yup.object({
  startDate: yup.string().required('날짜가 입력되지 않았습니다. 날짜는 필수 입력값입니다.'),

  endDate: yup.string().required('날짜가 입력되지 않았습니다. 날짜는 필수 입력값입니다.'),
...
const updateReservationPathValidator = yup.object({
  id: yup.string().matches(REGEX.EMPTY_VARIABLE_PATH, 'id는 필수 입력값입니다.'),
});
